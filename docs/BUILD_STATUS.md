...
- **A real CI pipeline now exists** (`.github/workflows/ci.yml`) — none did before. A `quality` job runs typecheck/lint/test/format/build on every push and PR; a `database-rls` job spins up a `postgres:16` GitHub Actions service container and runs the same migration + stand-in-schema + RLS-test sequence just verified locally. This directly automates the first bullet of `docs/SECURITY.md` §19's pre-release checklist (RLS tested) rather than leaving it as a manual, easy-to-skip step. A real local Supabase stack (`supabase start`) remains the more authoritative check before an actual release — see the test file's own header — but this catches regressions on every commit instead of only when someone remembers to run it by hand. **Known caveat, honestly flagged rather than assumed away:** the workflow's first live runs on GitHub show `conclusion: startup_failure` with zero jobs ever scheduled (confirmed via the Actions API — not a single job or step ran). That signature — failing before any job is even dispatched — points at GitHub Actions being disabled, spending-limited, or policy-restricted for this repository/organization, not a defect in `ci.yml` itself: the exact command sequence in both jobs was independently verified locally (see "Verification performed" below) using a clean/minimal shell environment. Whoever has admin access to the GitHub organization needs to check Settings → Actions (repo and org level) before this pipeline actually starts protecting anything.
