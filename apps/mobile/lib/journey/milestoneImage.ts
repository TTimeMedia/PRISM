...
export type PickMilestoneImageResult =
  | { status: 'picked'; asset: ImagePicker.ImagePickerAsset }
  | { status: 'denied' }
  | { status: 'canceled' };

/**
 * Milestone photos live in the private `memories` bucket under
 * `{user_id}/milestones/` (per-user RLS already exists — see
 * supabase/migrations). `milestones.image_path` holds the object path,
 * never a URL, the same convention as profile photos
 * (lib/you/profilePhoto.ts). Library-only; permission is requested only
 * when the user taps "Add photo".
 */
export async function pickMilestoneImage(): Promise<PickMilestoneImageResult> {
...
export async function uploadMilestoneImage(
  ...
  const path = `${userId}/milestones/${unique}.${extension}`;
...
export async function removeMilestoneImage(path: string | null | undefined): Promise<void> {
  if (!path) return;
  try {
